...
test("user can log in and tip from the match-first dashboard", async ({ page }, testInfo) => {
  ...
  await expect(page.locator(".daily-vm-moment")).toHaveCount(0);
  await expect(page.getByText("Dagens VM-øyeblikk")).toHaveCount(0);
  ...
  await page.goto("/vm");
  const vmDailyMoment = page.locator(".daily-vm-moment");
  await expect(vmDailyMoment).toBeVisible();
  await expect(vmDailyMoment.getByText("Dagens VM-øyeblikk")).toBeVisible();
  await expect(page.getByText("Dagens VM-øyeblikk")).toHaveCount(1);
  const vmDailyMomentImage = vmDailyMoment.locator(".daily-image-photo");
  const initialVmDailyMomentSrc = await vmDailyMomentImage.getAttribute("src");
  const vmDailyMomentShuffle = vmDailyMoment.getByRole("button", { name: "Vis nytt bilde til VM-øyeblikket" });
  await expect(vmDailyMomentShuffle).toBeVisible();
  await expect(vmDailyMomentShuffle).toContainText("Nytt bilde");
  await vmDailyMoment.locator(".daily-image-stage").click();
  await expect.poll(async () => vmDailyMomentImage.getAttribute("src")).toBe(initialVmDailyMomentSrc);
  const vmMomentImageContextButton = vmDailyMoment.getByRole("button", { name: /Vis bildekontekst/ }).first();
  await expect(vmMomentImageContextButton).toBeVisible();
  await expect(vmMomentImageContextButton).toContainText("Les bildetekst");
  await vmMomentImageContextButton.click();
  await expect.poll(async () => vmDailyMomentImage.getAttribute("src")).toBe(initialVmDailyMomentSrc);
  const vmMomentImageContextCard = vmDailyMoment.locator(".daily-image .image-context-card");
  await expect(vmMomentImageContextCard).toBeVisible();
  await expect(vmMomentImageContextCard).toContainText("Hva ser vi?");
  await expect(vmMomentImageContextCard).toContainText("Hvorfor betyr det noe?");
  await expect(vmMomentImageContextCard).toContainText("Nerdekrok");
  await expect(vmDailyMomentShuffle).toBeVisible();
  await expect(vmDailyMoment.locator(".image-context-backdrop")).toHaveCount(0);
  await expect.poll(async () => vmMomentImageContextCard.evaluate((element) => getComputedStyle(element).position)).not.toBe("fixed");
  await vmDailyMoment.locator(".image-context-close").click();
  await expect(vmMomentImageContextCard).toHaveCount(0);
  await vmDailyMomentShuffle.click();
  await expect.poll(async () => vmDailyMomentImage.getAttribute("src")).not.toBe(initialVmDailyMomentSrc);
  ...
  const vmArchiveImageWall = page.locator(".archive-image-wall-panel");
  const vmArchiveContextButton = vmArchiveImageWall.getByRole("button", { name: /Vis bildekontekst/ }).first();
